...
## Output Directory
ODIR = os.path.join(ROOT,'results','Supplement','Resubmit_1')
...
for i_ in range(len(df_BHM)):
	...
	h_, = ax2.plot(edh_u,Td_*1e-3,color=ci_,label='B: %.3f$\\pm$%.3f MPa a$^{1/3}$ O:%s'%(Bu*1e-6,Bs*1e-6,str(LVi)))
	handles.append(h_)
	ax2.fill_betweenx(Td_*1e-3,edh_sl,edh_su,color=ci_,alpha=0.25)
	if i_ > 0:
		ax1.text(df_BHD['edot_mea(a-1)'].values[i_-1] + 0.0001,df_BHD.index[i_-1] - 1,i_)
...
if issave:
	save_file = os.path.join(ODIR,'FigS5_Borehole_Deformation_%ddpi.png'%(DPI))
